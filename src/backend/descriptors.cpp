...
SourceTimeKind classify_src_time(const src_time *st) {
  if (dynamic_cast<const gaussian_src_time *>(st)) return SourceTimeKind::gaussian;
  if (dynamic_cast<const continuous_src_time *>(st)) return SourceTimeKind::continuous;
...
void build_source_descriptors(fields &f, SourcePlan &out) {
  ...
  for (const src_time *st = f.sources; st; st = st->next) {
    ...
    if (d.kind == SourceTimeKind::gaussian || d.kind == SourceTimeKind::continuous) {
      d.parameters.push_back(st->frequency().real());
      d.parameters.push_back(st->frequency().imag());
      d.parameters.push_back(st->get_fwidth());
    }
  ...
  for (int i = 0; i < f.num_chunks; ++i) {
    ...
    FOR_FIELD_TYPES(ft) {
      for (const src_vol &sv : fc.get_sources(ft)) {
        ...
        d.chunk = i;
        d.ft = ft;
        /* The destination is the E/H-family component of the same direction,
        ...
        d.destination =
            f.array_catalog->find({i, int(array_kind::f), int(d.c), 0, 0});
