...
CUDA_UCX_TLS = "self,sm,cuda_copy,cuda_ipc"
ROCM_UCX_TLS = "self,sm,rocm_copy,rocm_ipc"
# Backward-compatible name used by older callers and tests.
UCX_TLS = CUDA_UCX_TLS
COUNTER_AGGREGATIONS = {
    "messages_sent": "sum",
    "messages_received": "sum",
    "bytes_sent": "sum",
    "bytes_received": "sum",
    "device_to_host_calls": "sum",
    "device_to_host_bytes": "sum",
    "host_to_device_calls": "sum",
    ...
    "slot_reuses": "sum",
    "overlap_stages": "sum",
    "overlap_interior_launches": "sum",
    "overlap_boundary_launches": "sum",
    "graph_capture_count": "sum",
    "graph_launch_count": "sum",
    "graph_boundary_count": "sum",
    ...
    "gather_launches": "sum",
    "scatter_launches": "sum",
    "request_completions": "sum",
    "host_fallback_steady_capacity_growths": "sum",
    "material_recipe_prepare_nanoseconds": "sum",
    "material_initialize_nanoseconds": "sum",
    "graph_build_nanoseconds": "sum",
    "gather_pack_nanoseconds": "sum",
    "device_to_host_nanoseconds": "sum",
    "mpi_progress_nanoseconds": "sum",
    "mpi_wait_nanoseconds": "sum",
    "host_to_device_nanoseconds": "sum",
    "scatter_unpack_nanoseconds": "sum",
    "steady_allocation_count": "sum",
    "graph_recapture_count": "sum",
    "full_field_copy_count": "sum",
}
GAUGE_AGGREGATIONS = {
    "transport_device_bytes": "max",
    "transport_pinned_bytes": "max",
    "process_device_bytes_current": "max",
    "process_device_bytes_peak": "max",
    "process_pinned_bytes_current": "max",
    "process_pinned_bytes_peak": "max",
...
def configured_build_provenance(
    build_directory: pathlib.Path,
    mpiexec: pathlib.Path,
    python: pathlib.Path,
    accelerator: str = "cuda",
    toolkit_compiler: pathlib.Path | None = None,
    inventory_tool: pathlib.Path | None = None,
) -> dict[str, Any]:
    ...
    command_environment["PATH"] = (
        str(mpiexec.parent) + os.pathsep + command_environment.get("PATH", "")
    )
    ...
    configure_arguments = required_output(
        [str(config_status), "--config"], "configure arguments"
    )
    compiler = (
        pathlib.Path(flags["CXX"].split()[0])
        if flags["CXX"]
        else pathlib.Path("mpic++")
    )
    ...
    if toolkit_compiler is None:
        configured = flags["HIPCC"] if accelerator == "hip" else "nvcc"
        toolkit_compiler = pathlib.Path(
            configured.split()[0] if configured else accelerator
        )
    if not toolkit_compiler.is_absolute():
        resolved_tool = shutil.which(
            str(toolkit_compiler), path=command_environment.get("PATH")
        )
        if not resolved_tool:
            raise RunnerError(f"cannot find {accelerator} compiler {toolkit_compiler}")
        toolkit_compiler = pathlib.Path(resolved_tool)
    toolkit_version = required_output(
        [str(toolkit_compiler), "--version"], f"{accelerator} toolchain version"
    )
    inventory = None
    if inventory_tool is not None:
        inventory_tool = inventory_tool.resolve()
        inventory = {
            "tool": str(inventory_tool),
            "sha256": bm.sha256_file(inventory_tool),
    ...
    return {
        ...
        "accelerator_toolchain": toolkit_version,
        "accelerator_inventory": inventory,
        # Retained for schema-v2 compatibility with existing CUDA artifacts.
        "cuda_toolkit": toolkit_version if accelerator == "cuda" else None,
        "executable_sha256": {
...
def observed_cpu_numa_nodes(affinity: Sequence[int]) -> list[int]:
    nodes: set[int] = set()
    for cpu in affinity:
        for path in pathlib.Path(f"/sys/devices/system/cpu/cpu{cpu}").glob(
            "node[0-9]*"
        ):
            nodes.add(int(path.name[4:]))
    if not nodes:
        raise RunnerError("cannot determine a NUMA node for the observed CPU affinity")
    return sorted(nodes)
...
def python_purelib(python: pathlib.Path) -> pathlib.Path:
    ...
    try:
        ...
        raise RunnerError(
            f"cannot query the selected Python installation: {error}"
        ) from error
    if not output:
...
def _hip_pci_bus_id(logical_device_id: int) -> str:
    ...
    return normalize_pci_bus_id(value.value.decode("ascii"))
...
def _rocm_device_provenance(
    ...
    matches = [
        ...
        and normalize_pci_bus_id(str(entry.get("PCI Bus", ""))) == runtime_bus_id
    ]
    if len(matches) != 1:
        raise RunnerError(
    ...
    memory_text = first_matching("vram", "total")
    memory_match = re.search(r"([0-9]+)", memory_text)
    return {
        "accelerator": "hip",
        "visible_device": device_id,
        "visible_devices": list(selectors),
        "process_device_id": device_id,
        # `rocm-smi` card indices are re-enumerated under ROCR masking.  Keep
        # the explicit unmasked selector from the launch as the physical
        # identity and record the inventory key separately.
        "physical_selector": str(selectors[device_id]),
        "inventory_card": physical_name,
        "uuid": runtime_uuid,
...
def accelerator_device_provenance(
    accelerator: str,
    device_id: int,
    runtime_uuid: str,
    selectors: Sequence[str],
    rocm_smi: pathlib.Path | None,
) -> dict[str, Any]:
    if accelerator == "hip":
        if rocm_smi is None:
            raise RunnerError("HIP provenance requires --rocm-smi")
        return _rocm_device_provenance(device_id, runtime_uuid, selectors, rocm_smi)
    device = legacy._nvidia_device_provenance(device_id)[0]
    device["accelerator"] = "cuda"
    device["visible_devices"] = list(selectors)
    return device
...
def launch_environment(
    ...
    if route not in {"none", "staged", "direct", "auto"}:
        raise RunnerError("route must be none, staged, direct, or auto")
    if accelerator not in {"cuda", "hip"}:
        raise RunnerError("accelerator must be cuda or hip")
    env = dict(base)
    env.update(
        {
            "OMPI_MCA_pml": "ucx",
            "UCX_TLS": CUDA_UCX_TLS if accelerator == "cuda" else ROCM_UCX_TLS,
            "MEEP_GPU_AWARE_MPI": {
                "none": "no",
                "staged": "no",
                "direct": "yes",
            ...
            "MEEP_ACCELERATOR_RUNTIME": accelerator,
        }
    )
    if accelerator == "cuda":
...
def derive_observables(
    ...
    for name, definition in definitions.items():
        if definition.get("normalization", {}).get("kind") == "mode_power_ratio":
            ...
            if len(numerator) != len(wavelengths) or len(denominator) != len(
                wavelengths
            ):
                raise RunnerError(
                    "monitor array length disagrees with wavelength sampling"
                )
    ...
    if any(
        policy["evaluation"].startswith("not_a_knot")
        ...
        try:
            ...
            raise RunnerError(
                "ring observable evaluation requires NumPy and SciPy"
            ) from error
        ...
        sample_y = CubicSpline(
            wavelengths, normalized, bc_type="not-a-knot", extrapolate=False
        )(sample_x)
        minimum_index = int(np.argmin(sample_y))
        half = 0.5 * (1.0 + float(sample_y[minimum_index]))
        ...
        left = crossing(range(0, minimum_index))
        right = crossing(range(minimum_index, len(sample_x) - 1))
        width = right - left
        if width <= 0:
            raise RunnerError("ring FWHM is not positive")
        resonance = 0.5 * (left + right)
        ring_values = {
            "resonance_wavelength_um": resonance,
            "fwhm_um": width,
    ...
    for policy in manifest["validation_policy"]["required_observables"]:
        ...
        if evaluation == "linear_center_wavelength_mode_power_ratio":
            ...
            evaluation == "negative_ten_log10_linear_center_wavelength_mode_power_ratio"
        ):
        ...
        allowed = policy["absolute_tolerance"] + policy["relative_tolerance"] * abs(
            reference["value"]
        )
        output.append(
            {
...
def advance_with_collective_stop(
    ...
    if (
        ...
        raise RunnerError(
            "collective point-field decay requires component and three-vector point"
        )
    decay = mp.stop_when_fields_decayed(
        float(stopping["check_interval_meep_time"]),
        getattr(mp, component_name),
        mp.Vector3(*point),
...
def reconcile_rank_records(
    ...
    module_hashes = set()
    singleton_no_transport = size == 1 and requested["route"] == "none"
    for record in records:
        ...
        identities.add(
            (record.get("communicator_size"), record.get("communicator_generation"))
        )
        ...
        requested_transport = "staged" if singleton_no_transport else requested["route"]
        for key, expected in (
            ...
            if runtime.get(key) != expected:
                raise RunnerError(
                    f"rank {record['rank']} {key} disagrees with the request"
                )
        if (
            runtime.get("communicator_rank") != record["rank"]
            or runtime.get("communicator_size") != size
        ):
            raise RunnerError(
                "runtime report communicator identity disagrees with its rank record"
            )
        if (
            runtime.get("counter_scope") != "rank_local_current_epoch"
            or runtime.get("backend_counter_scope") != "rank_local_backend_lifetime"
            or runtime.get("memory_gauge_scope") != "rank_local_process_lifetime"
        ):
            raise RunnerError("runtime counter scope is not rank-local current-epoch")
        if (
            runtime.get("setup_counter_scope") != "rank_local_current_backend_state"
            or runtime.get("transport_timing_scope")
            != "rank_local_current_transport_epoch_host_elapsed"
            or runtime.get("allocation_counter_scope") != "rank_local_process_lifetime"
        ):
            raise RunnerError("runtime timing/allocation counter scope is invalid")
        if singleton_no_transport:
            if (
                runtime.get("captured_transport_epoch_active")
                or runtime.get("captured_transport_epoch_fresh")
                or runtime.get("captured_requested_transport") != "none"
            ):
                raise RunnerError(
                    "singleton result unexpectedly captured a transport epoch"
                )
        else:
            if not runtime.get("captured_transport_epoch_active") or not runtime.get(
                "captured_transport_epoch_fresh"
            ):
                raise RunnerError("runtime transport epoch is absent or stale")
            if (
                runtime.get("captured_requested_transport") != requested["route"]
                or runtime.get("captured_overlap_policy") != requested["overlap"]
            ):
                raise RunnerError(
                    "captured transport policy disagrees with the request"
                )
        if not runtime.get("device_owner") or record.get("role") != "owner":
            raise RunnerError(
                "the normative runner requires one GPU-owning rank per process"
            )
    ...
    if repetition_count < 1 or any(
        len(r["repetitions"]) != repetition_count for r in records
    ):
    ...
    for index in range(repetition_count):
        values = [
            _number(r["repetitions"][index]["steady_seconds"], "steady_seconds")
            for r in records
        ]
    ...
    for name, aggregation in COUNTER_AGGREGATIONS.items():
        ...
        for record in records:
            ...
            for repetition in record["repetitions"]:
                if (
                    set(repetition["counter_start"]) != set(COUNTER_AGGREGATIONS)
                    or set(repetition["counter_end"]) != set(COUNTER_AGGREGATIONS)
                    or set(repetition["counter_deltas"]) != set(COUNTER_AGGREGATIONS)
                ):
                    raise RunnerError(
                        "counter snapshots do not contain the declared counter set"
                    )
                delta = (
    ...
    if (
        len({record["runtime"]["captured_provider_signature"] for record in records})
        != 1
    ):
        raise RunnerError("captured MPI provider signature disagrees across ranks")
    for requested_key, resolved_key in (
        ("precision", "resolved_precision"),
        ("route", "resolved_transport"),
    ):
        if (
            requested[requested_key] != "auto"
            and resolved[resolved_key] != requested[requested_key]
        ):
    ...
    shapes = {
        tuple(rep["grid_shape"]) for record in records for rep in record["repetitions"]
    }
    ...
    if len(shapes) != 1 or len(steps) != 1 or len(dts) != 1:
        raise RunnerError(
            "simulation shape/step/timestep disagrees across ranks or repetitions"
        )
    for index in range(repetition_count):
    ...
    return {
        "communicator": {
            "size": size,
            "generation": records[0]["communicator_generation"],
        },
        "resolved_execution": {
            "backend": resolved["resolved_backend"],
            "precision": resolved["resolved_precision"],
            "route": resolved["resolved_transport"],
            "overlap": resolved["resolved_overlap"],
        ...
        "performance": {
            "grid_timesteps_per_second": math.prod(shape)
            * step_count
...
def validate_result(
    ...
    *,
    ...
    accelerator = launch.get(
        "accelerator", environment.get("MEEP_ACCELERATOR_RUNTIME", "cuda")
    )
    if accelerator not in {"cuda", "hip"}:
        raise RunnerError("result accelerator provenance is invalid")
    expected_provider = {
        "OMPI_MCA_pml": "ucx",
        "UCX_TLS": CUDA_UCX_TLS if accelerator == "cuda" else ROCM_UCX_TLS,
    }
    if accelerator == "cuda":
        expected_provider["OMPI_MCA_opal_cuda_support"] = "true"
    if any(environment.get(key) != value for key, value in expected_provider.items()):
        raise RunnerError(
            "result does not record the authoritative UCX provider environment"
        )
    if accelerator == "hip" and "OMPI_MCA_opal_cuda_support" in environment:
        raise RunnerError("HIP result contains the CUDA-aware Open MPI override")
    visibility_name = (
        "ROCR_VISIBLE_DEVICES" if accelerator == "hip" else "CUDA_VISIBLE_DEVICES"
    )
    if "accelerator" in launch:
        if environment.get("MEEP_ACCELERATOR_RUNTIME") != accelerator:
            raise RunnerError("launch accelerator and environment provenance disagree")
        if environment.get(visibility_name) != ",".join(
            launch.get("visible_devices", [])
        ):
            raise RunnerError(
                "recorded accelerator visibility does not match the launch"
            )
        provenance = result["provenance"]
        if provenance.get("accelerator_runtime") != accelerator or not provenance.get(
            "accelerator_toolchain"
        ):
            raise RunnerError("accelerator toolchain provenance is incomplete")
        if accelerator == "hip" and not isinstance(
            provenance.get("accelerator_inventory"), Mapping
        ):
            raise RunnerError("HIP result lacks ROCm inventory provenance")
    if (
        pathlib.Path(launch["argv"][0]) != pathlib.Path(launch["mpiexec"])
        or launch["python"] not in launch["argv"]
    ):
        raise RunnerError("result launcher argv disagrees with its recorded prefix")
    requested = result["requested_execution"]
    expected_policy = {"none": "no", "staged": "no", "direct": "yes", "auto": "auto"}[
        requested["route"]
    ]
    ...
    for key, environment_name in (
        ("overlap", "MEEP_NVIDIA_MPI_OVERLAP"),
        ("graph", "MEEP_NVIDIA_GRAPH_MODE"),
        ("precision", "MEEP_PRECISION"),
    ):
        if environment.get(environment_name) != requested[key]:
            raise RunnerError(
                f"recorded {environment_name} contradicts requested execution"
            )
    reconciled = reconcile_rank_records(
        result["rank_records"], result["run_manifest"]["sha256"], requested
    )
    for key in (
        "communicator",
        "resolved_execution",
    ...
    if requested["route"] == "direct" and not (
        runtime["mpi_query_available"] and runtime["mpi_cuda_aware"]
    ):
        raise RunnerError(
            "direct result lacks a positive CUDA-aware or ROCm-aware MPI provider query"
        )
    uuids = [
    ...
    for record in result["rank_records"]:
        if record["role"] == "owner":
            ...
            if (
                not isinstance(record["device"], Mapping)
                or not required_device.issubset(record["device"])
                or normalize_gpu_uuid(record["device"].get("uuid"))
    ...
    if any(
        ...
        raise RunnerError(
            "rank launch/provenance hashes do not bind the published records"
        )
    for record in result["rank_records"]:
    ...
    if accelerator == "hip":
        visible_devices = launch.get("visible_devices")
        if (
            not isinstance(visible_devices, list)
            or len(visible_devices) != result["requested_execution"]["ranks"]
            or len(set(visible_devices)) != len(visible_devices)
        ):
            raise RunnerError(
                "HIP result does not record one distinct visible selector per rank"
            )
        bdfs, physical_ids = set(), set()
        for record in result["rank_records"]:
            ...
            if (
                not required_hip.issubset(device)
                or device["accelerator"] != "hip"
                or device["visible_devices"] != visible_devices
                or str(device["physical_selector"])
                != str(visible_devices[record["local_rank"]])
                or not device["physical_unique_id"]
                or not device["architecture"]
            ):
                raise RunnerError(f"rank {record['rank']} HIP provenance is incomplete")
            bdf = normalize_pci_bus_id(device["pci_bus_id"])
            bdfs.add(bdf)
            physical_ids.add(str(device["physical_unique_id"]))
            numa_nodes = record.get("cpu_numa_nodes")
            if numa_nodes != [device["numa_node"]]:
                raise RunnerError(
                    f"rank {record['rank']} CPU/GPU NUMA placement is not local"
                )
        if len(bdfs) != len(result["rank_records"]) or len(physical_ids) != len(
            result["rank_records"]
        ):
            raise RunnerError("HIP owner ranks do not map to unique physical GPUs")
    ...
    for record in result["rank_records"]:
        ...
        if any(per_rank[name] for name in forbidden_steady_counters):
            raise RunnerError(
                f"rank {record['rank']} recorded allocation, recapture, copy, or fallback "
                "during the measured interval"
            )
        if route == "direct":
            if not runtime["mpi_query_available"] or not runtime["mpi_cuda_aware"]:
                raise RunnerError(
                    f"rank {record['rank']} direct route lacks a positive CUDA-aware or "
                    "ROCm-aware MPI query"
                )
            if (
                any(
                    per_rank[name]
                ...
                or (
                    result["requested_execution"]["ranks"] > 1
                    and per_rank["direct_bytes"] <= 0
                )
                or per_rank["direct_bytes"]
            ...
            if (
                per_rank["direct_bytes"] != 0
                or per_rank["device_to_host_calls"] <= 0
                or per_rank["host_to_device_calls"] <= 0
                or per_rank["device_to_host_bytes"] != per_rank["bytes_sent"]
                or per_rank["host_to_device_bytes"] != per_rank["bytes_received"]
                or runtime["transport_pinned_bytes"] <= 0
            ):
                raise RunnerError(
        ...
        if result["resolved_execution"]["graph"] == "graph" and (
            not runtime["graph_enabled"]
            or not runtime["graph_valid"]
            or per_rank["graph_launch_count"] <= 0
        ):
            raise RunnerError(
    ...
    if route == "direct":
        ...
        if (
            ...
            or counters["transport_pinned_bytes"] <= 0
        ):
    ...
    if manifest is not None and references is not None:
        ...
        expected_requested = {
            "backend": requested_manifest["backend"],
        ...
        if result["requested_execution"] != expected_requested:
            raise RunnerError(
                "published requested execution does not match the manifest"
            )
        expected_repetitions = manifest["execution"]["measured_repetitions"]
        if any(
            len(record["repetitions"]) != expected_repetitions
            for record in result["rank_records"]
        ):
        ...
        for policy in manifest["validation_policy"]["required_observables"]:
            reference = references[policy["name"]]
            if (
                reference["monitor"] != policy["monitor"]["name"]
                or reference["unit"] != policy["unit"]
            ):
                raise RunnerError(
                    f"physics reference {policy['name']} monitor/unit is not canonical"
                )
        expected = []
        for repetition in result["rank_records"][0]["repetitions"]:
            expected.append(
                derive_observables(manifest, repetition["monitors"], references)
            )
        published = result["physics_observables"]
        if [item["name"] for item in published] != [
            item["name"] for item in expected[0]
        ]:
        ...
        for index, item in enumerate(published):
            ...
            candidate = dict(expected[0][index])
            candidate["values_by_repetition"] = values
            candidate["value"] = statistics.median(values)
            candidate["passed"] = all(group[index]["passed"] for group in expected)
            if item != candidate:
                raise RunnerError(
                    f"published physics observable {item['name']} is not derived from raw monitors"
                )


def authenticate_result_files(
    result: Mapping[str, Any]
) -> tuple[dict[str, Any], dict[str, Mapping[str, Any]]]:
    manifest, manifest_hash = manifest_snapshot(
        pathlib.Path(result["run_manifest"]["path"])
    )
...
def _worker(args: argparse.Namespace) -> int:
    ...
    if gather is None:
        raise RunnerError(
            "Meep lacks active_communicator_allgather_json(payload: str) -> list[str]"
        )
    try:
        ...
        if manifest["result_schema"]["schema_version"] != 2:
            raise RunnerError(
                "multi-rank execution requires the normative result schema version 2"
            )
        reference, reference_digest = manifest_snapshot(args.physics_reference)
        if reference_digest != args.physics_reference_sha256:
            raise RunnerError(
                "physics reference changed between launch and worker startup"
            )
        references = bm._load_physics_reference(
            args.physics_reference, reference_digest, details
        )
        ...
        manifest_phase = {
            ...
            "physics_reference_sha256": None,
            "requested": None,
        }
    manifest_phases = [
        json.loads(item) for item in gather(json.dumps(manifest_phase, sort_keys=True))
    ]
    phase_errors = [
        f"rank {index}: {item.get('error')}"
        for index, item in enumerate(manifest_phases)
        if not item.get("ok")
    ]
    if phase_errors:
        raise RunnerError(
            "collective manifest phase failed: " + "; ".join(phase_errors)
        )
    if {item["manifest_sha256"] for item in manifest_phases} != {args.manifest_sha256}:
        raise RunnerError(
            "immutable manifest hash disagrees during collective preflight"
        )
    if {item["physics_reference_sha256"] for item in manifest_phases} != {
        args.physics_reference_sha256
    }:
        raise RunnerError(
            "physics reference hash disagrees during collective preflight"
        )
    if any(
        item["requested"] != manifest_phases[0]["requested"]
        for item in manifest_phases[1:]
    ):
        raise RunnerError("requested execution disagrees during collective preflight")
    if not divided and details["requested"]["backend"] == "nvidia":
        # Accelerator compilation requires a communicator context even for a
        # singleton manifest whose transport is deliberately `none`.
        mp.begin_global_communications()
        mp.divide_parallel_processes(1)
        divided = True
    try:
        records, translation = legacy._geometry_records(manifest, gdstk)
        planes = legacy.validate_planes(
            manifest, legacy.transformed_ports(manifest, translation)
        )
        geometry_phase = {"ok": True, "error": None}
    except Exception as error:
        geometry_phase = {"ok": False, "error": f"{type(error).__name__}: {error}"}
    geometry_phases = [
        json.loads(item) for item in gather(json.dumps(geometry_phase, sort_keys=True))
    ]
    phase_errors = [
        f"rank {index}: {item.get('error')}"
        for index, item in enumerate(geometry_phases)
        if not item.get("ok")
    ]
    if phase_errors:
        raise RunnerError(
            "collective geometry phase failed: " + "; ".join(phase_errors)
        )
    try:
        repetitions = []
        runtime = None
        for repetition_index in range(manifest["execution"]["measured_repetitions"]):
            try:
                started = time.perf_counter()
                simulation, monitors = legacy._build_simulation(
                    mp, manifest, records, planes, device_id=_local_rank()
                )
                initialization = time.perf_counter() - started
                warmup = manifest["execution"]["warmup_steps"]
                warmup_started = time.perf_counter()
                if warmup:
                    simulation.fields.advance(warmup)
                ...
                monitor_values = legacy._monitor_output(
                    mp, simulation, monitors, manifest
                )
                monitor_seconds = time.perf_counter() - monitor_started
                counter_deltas = {
                    name: int(runtime[name]) - int(initial_runtime[name])
                    for name in COUNTER_AGGREGATIONS
                }
                ...
                repetition = {
                    ...
                    "grid_shape": legacy._grid_shape(simulation),
                    "stop_reason": stop_reason,
                    "counter_start": {
                        name: int(initial_runtime[name])
                        for name in COUNTER_AGGREGATIONS
                    },
                    "counter_end": {
                        name: int(runtime[name]) for name in COUNTER_AGGREGATIONS
                    },
            ...
            repetition_phases = [
                json.loads(item)
                for item in gather(
                    json.dumps(repetition_phase, sort_keys=True, allow_nan=False)
                )
            ]
            phase_errors = [
                f"rank {index}: {item.get('error')}"
                for index, item in enumerate(repetition_phases)
                if not item.get("ok")
            ]
            if phase_errors:
                raise RunnerError(
                    f"collective repetition {repetition_index} failed: "
                    + "; ".join(phase_errors)
                )
            local_phase = repetition_phases[int(mp.my_rank())]
            repetitions.append(local_phase["repetition"])
            runtime = local_phase["runtime"]
        if runtime is None:
        ...
        device = (
            accelerator_device_provenance(
                args.accelerator,
                int(runtime["device_id"]),
                str(runtime["device_uuid"]),
                selectors,
                args.rocm_smi,
            )
            if runtime["device_owner"]
            else None
        )
        ...
        local = {
            "rank": rank,
            "communicator_size": int(runtime["communicator_size"]),
            ...
            "role": "owner" if runtime["device_owner"] else "idle",
            "device": device,
            "runtime": runtime,
            "repetitions": repetitions,
            "module_paths": {
                "meep": str(meep_module),
                "extension": str(meep_extension),
            },
            "module_sha256": {
                "meep": bm.sha256_file(meep_module),
                "extension": bm.sha256_file(meep_extension),
            },
            "cpu_affinity": observed_cpu_affinity(),
            "cpu_numa_nodes": observed_cpu_numa_nodes(observed_cpu_affinity()),
        }
    ...
    envelopes = [
        json.loads(item)
        for item in gather(json.dumps(envelope, sort_keys=True, allow_nan=False))
    ]
    errors = [
        f"rank {index}: {item.get('error')}"
    ...
    rank = int(mp.my_rank())
    requested = {
        "backend": details["requested"]["backend"],
        "precision": args.precision,
        "route": args.route,
        "overlap": details["requested"]["overlap"],
        "graph": details["requested"]["graph"],
        "ranks": details["requested"]["ranks"],
    }
    reconciled = reconcile_rank_records(gathered, digest, requested)
    per_repetition_observables = [
        derive_observables(
            manifest, gathered[0]["repetitions"][index]["monitors"], references
        )
    ...
    for index, observable in enumerate(per_repetition_observables[0]):
        ...
        allowed = float(merged["absolute_tolerance"]) + float(
            merged["relative_tolerance"]
        ) * abs(reference_value)
        merged["passed"] = all(
            abs(value - reference_value) <= allowed for value in values
        )
    ...
    if rank == 0:
        result = {
            ...
            "physics_reference": {
                "path": str(args.physics_reference.resolve()),
            ...
            "provenance": json.loads(
                args.provenance_record_file.read_text(encoding="utf-8")
            ),
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument(
        "--route", required=True, choices=("none", "staged", "direct", "auto")
    )
    ...
    value.add_argument("--accelerator", choices=("cuda", "hip"), default="cuda")
    value.add_argument(
        "--visible-devices",
        help="comma-separated CUDA or ROCr physical selectors, one per local rank",
    )
    value.add_argument(
        "--cuda-visible-devices",
        help="deprecated CUDA-only spelling of --visible-devices",
    )
    value.add_argument("--python", type=pathlib.Path)
    value.add_argument("--mpiexec", type=pathlib.Path)
    value.add_argument("--pythonpath", type=pathlib.Path)
    value.add_argument("--library-path", type=pathlib.Path)
    value.add_argument(
        "--runtime-library-path",
        action="append",
        type=pathlib.Path,
        default=[],
        help="runtime library directory (repeat in dynamic-loader order)",
    )
    ...
    value.add_argument("--map-by", default="ppr:1:package:PE=8")
    value.add_argument("--rank-by")
    value.add_argument("--bind-to", default="core")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        requested = details["requested"]
        if (
            requested["mpi_transport"] != args.route
            or requested["precision"] != args.precision
            or requested["overlap"] != args.overlap
            or requested["graph"] != args.graph
        ):
            raise RunnerError(
                "runner options do not match the manifest execution policies"
            )
        if requested["ranks"] not in {1, 2, 4, 8}:
            raise RunnerError("runner supports exactly 1, 2, 4, or 8 owning ranks")
        if args.visible_devices and args.cuda_visible_devices:
            raise RunnerError(
                "use only one of --visible-devices and --cuda-visible-devices"
            )
        visible_text = args.visible_devices or args.cuda_visible_devices
        if not visible_text:
            raise RunnerError("an explicit --visible-devices mapping is required")
        selectors = [item.strip() for item in visible_text.split(",") if item.strip()]
        if len(selectors) != requested["ranks"] or len(set(selectors)) != len(
            selectors
        ):
            raise RunnerError(
                "visible devices must contain one distinct selector per rank"
            )
        if args.accelerator == "hip" and args.rocm_smi is None:
            raise RunnerError("HIP benchmark provenance requires --rocm-smi")
        # Preserve virtual-environment launcher symlinks: resolving `venv/bin/python`
        # to the system interpreter silently drops the venv's site-packages.
        mpiexec = (
            args.mpiexec.absolute()
            if args.mpiexec
            else args.prefix.absolute() / "bin/mpirun"
        )
        python = (
            args.python.absolute()
            if args.python
            else args.prefix.absolute() / "bin/python"
        )
        if not mpiexec.is_file() or not python.is_file():
            raise RunnerError(
                "the selected MPI launcher or Python executable does not exist"
            )
        ...
        env[visibility_name] = ",".join(selectors)
        site_packages = (
            str(args.pythonpath.resolve())
            if args.pythonpath
            else str(python_purelib(python))
        )
        env["PYTHONPATH"] = site_packages + (
            (":" + env["PYTHONPATH"]) if env.get("PYTHONPATH") else ""
        )
        runtime_library_paths = [path.resolve() for path in args.runtime_library_path]
        if args.library_path:
            runtime_library_paths.insert(0, args.library_path.resolve())
        if not runtime_library_paths:
            runtime_library_paths = [args.prefix.resolve() / "lib"]
        ...
        runtime_library_paths = list(dict.fromkeys(runtime_library_paths))
        ...
        if args.accelerator == "cuda":
            environment_names = ("OMPI_MCA_opal_cuda_support",) + environment_names
        env_record = {name: env[name] for name in environment_names}
        ...
        with tempfile.NamedTemporaryFile(
            ...
            command += [
                ...
                "--physics-reference",
                str(args.physics_reference.resolve()),
                "--route",
                args.route,
                "--overlap",
                args.overlap,
            ...
            launch = {
                ...
                "ucx_version": legacy._command_output(
                    [str(args.ucx_info.resolve()), "-v"]
                    if args.ucx_info
                    else (
                        [str(args.prefix.resolve() / "bin/ucx_info"), "-v"]
                        if (args.prefix.resolve() / "bin/ucx_info").is_file()
                        else ["ucx_info", "-v"]
                    )
                ),
            }
